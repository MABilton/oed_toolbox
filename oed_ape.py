...
def find_optimal_d(sample_likelihood, sample_prior, log_probs_and_grads, d_bounds, num_repeats=9):
    best_ape = inf
...
def minimise_ape(log_probs_and_grads, sample_prior, sample_likelihood, d_bounds, num_samples=10, max_iter=100):
...
def adagrad_update(adagrad_s, grad, lr=5*10**-2, eps=10**-7):
    delta_d = -lr*grad/((adagrad_s + eps)**0.5)
    return delta_d

def check_loop_conditions(ape_store, num_iter, max_iter, ape_change_threshold=10**-3):
